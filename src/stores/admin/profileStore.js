...
const useProfileStore = create((set, get) => ({
  ...
  updateImage: async (file) => {
    ...
    try {
      const response = await axiosInstance.put('/api/admin/profile/image', formData, {
  ...
  updateDetails: async (details) => {
    set({ isLoading: true });
    try {
      const response = await axiosInstance.put('/api/admin/profile/profileDetails', details);
      set({ isLoading: false });
      return { success: true, data: response.data.data };
    } catch (err) {
      set({ isLoading: false });
  ...
  processUpi: async (upiId) => {
    set({ isLoading: true });
    try {
      const response = await axiosInstance.put('/api/admin/profile/upiDetails', { upiId });
      set({ isLoading: false });
      return { success: true, data: response.data.data };
    } catch (err) {
      set({ isLoading: false });
