...
const menuItems = [
  ...
  { name: "All Banks", icon: BuildingLibraryIcon,path:"/allbanks" },
  { name: "KYC Verification", icon: IdentificationIcon,path:"/kycverification" },
  { name: "All Tickets", icon: TicketIcon,path:"/alltickets" },
  { name: "Your Profile", icon: BellIcon,path:"/yourprofile" },
];
      ...
      <ul className="flex-1">
        {menuItems.map((item, index) => (
          <li onClick={()=>navigate(item.path)}
            key={index}
            className="flex items-center gap-x-4 p-2 mt-2 rounded-md cursor-pointer hover:bg-gray-800 transition"
          >
