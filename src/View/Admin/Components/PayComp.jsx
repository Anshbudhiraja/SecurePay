  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Scanner Card */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col items-center justify-center hover:border-green-500/50 transition">
          {!showScanner ? (
            <>
              <div className="w-48 h-48 border-2 border-dashed border-zinc-800 rounded-xl flex items-center justify-center mb-4 bg-zinc-800/20">
                <span className="text-zinc-500 text-sm text-center px-4">Scan UPI QR Code</span>
              </div>
              <button onClick={() => setShowScanner(true)} className="px-6 py-2 bg-green-600 hover:bg-green-500 rounded-lg font-semibold text-white">
                Open Camera
              </button>
            </>
          ) : (
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2 text-white text-2xl font-bold outline-none focus:border-green-500" />
            </div>
          </div>
          <button onClick={() => setShowConfirm(true)} disabled={!upiId || !amount || isLoading}
            className="w-full mt-6 py-3 rounded-xl font-bold bg-green-600 hover:bg-green-500 disabled:bg-zinc-800 disabled:text-zinc-600 transition-all">
            Pay ₹{amount || "0"}
          </button>
        </div>
      </div>
