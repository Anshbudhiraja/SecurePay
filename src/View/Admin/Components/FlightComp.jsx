...
const FlightComp = () => {
  ...
  const handleSearch = () => {
    searchTickets({ source, destination, name, ticketType: "Flight" });
  };
  useEffect(()=>{
      searchTickets({ source, destination, name, ticketType: "Flight" });
    },[searchTickets])
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <div className="mb-6">
        ...
        <p className="text-zinc-400">Search for available flights across the network</p>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <input placeholder="Destination" value={destination} onChange={(e) => setDestination(e.target.value)}
            className="bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2 text-white" />
          <input placeholder="Flight Name" value={name} onChange={(e) => setName(e.target.value)}
            className="bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-2 text-white" />
      ...
      <div className="space-y-4">
        {tickets.length === 0 ? (
          <p className="text-zinc-500 text-center py-10">No flight found. Try changing your search filters.</p>
        ) : (
          tickets.map((train) => (
